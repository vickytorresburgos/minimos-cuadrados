...
# init(autoreset=True) #inicializa colorama y reestablece automaticamente los colores depsues de cada print

# def main(): #define funcion principal 

#         x = list(map(float, input(Fore.YELLOW + "Ingrese los valores de x separados por comas: ").split(','))) #define x
#         y = list(map(float, input(Fore.YELLOW + "Ingrese los valores de y separados por comas: ").split(','))) #define y 
#         x = np.array(x) #Estos valores se convierten a listas de float y luego a numpy arrays.
#         y = np.array(y)
...
#             if opcion == '1':
#                 bases = input("Ingrese las bases polinómicas separadas por comas").split(',')
...
#                     exp_form = input(Fore.CYAN + "Seleccione una opción (1-3): ").strip()
